...
void main() {
  final Map<String, List<List<dynamic>>> productsData = {
    'Vegetables & Fruits': [
  ...
  for (final cat in productsData.keys) {
    final int catIndex = productsData.keys.toList().indexOf(cat) + 1;
    final shelf = "S$catIndex";
    final items = productsData[cat]!;

    for (var item in items) {
      final String name = item[0];
      final price = item[1];
      final String unit = item[2];
      final String emoji = item[3];
      final String description = item[4];
      final String imageUrl = item[5];

      final tempStr = idCounter.toString().padLeft(3, '0');
      final pid = "PRD$tempStr";
      int stock = rnd.nextInt(96) + 5; // 5 to 100
      
      sink.writeln('      {"id": "$pid", "name": "$name", "category": "$cat", "price": $price.0, "stock": $stock, "threshold": 20, "emoji": "$emoji", "shelf": "$shelf", "unit": "$unit", "description": "$description", "imageUrl": "$imageUrl", "supplierId": "SUP001"},');
      idCounter++;
    }
  }
